...
export function buildAuthzVerifyResult({ release, bundle_ok = true, authz_verification }) {
  ...
  if (av.scope) {
    ...
    assert(Number.isInteger(av.scope.permission_count) && av.scope.permission_count >= 0, 'scope.permission_count must be integer >= 0');
    assert(Number.isInteger(av.scope.role_count) && av.scope.role_count >= 0, 'scope.role_count must be integer >= 0');
    ...
    assert(typeof av.scope.deny_by_default === 'boolean', 'scope.deny_by_default must be boolean');
    assert(av.scope.deny_by_default_scope === 'route_registry_only', 'scope.deny_by_default_scope must be "route_registry_only"');
  }
