...
export async function GET(req: NextRequest) {
  ...
  const apiBase = (process.env.PORTAL_API_URL || '').trim() || (process.env.NEXTAUTH_URL || `http://localhost:${process.env.PORT || 3001}`).replace(/\/$/, '') + '/api';
  try {
    const url = `${apiBase.replace(/\/+$/, '')}/v1/docs/versions?doc_id=${encodeURIComponent(docId)}&limit=${encodeURIComponent(limit)}`;
